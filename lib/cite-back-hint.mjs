// qwen-mem-lite: PostToolUse cite-back hint builder.
//
...
const OBS_INSIGHT_SAVE_RE = /(?:cli\.mjs|qwen-mem-lite)['"]?\s+save\b[\s\S]*?--lesson(?:-learned)?\b/;
...
//     QWEN_MEM_CITE_NUDGE_WIDE_DENOMINATOR=1.
...
// QWEN_MEM_CITE_NUDGE_SILENCE_AFTER (0 = never silence).
 ...
 * QWEN_MEM_CITE_NUDGE_WIDE_DENOMINATOR=1 plus QWEN_MEM_CITE_NUDGE_THRESHOLD=0.6.
 */
function gateStats(data, env) {
  ...
  if (env.QWEN_MEM_CITE_NUDGE_WIDE_DENOMINATOR === '1') return wide;
...
function ratioGateFires(rawData, env) {
  ...
  const threshold = envNumber(env.QWEN_MEM_CITE_NUDGE_THRESHOLD, {
    name: 'QWEN_MEM_CITE_NUDGE_THRESHOLD',
    defaultValue: 0.4,
    min: 0,
    max: 1,
  });
  ...
  const minInjected = envNumber(env.QWEN_MEM_CITE_NUDGE_MIN_INJECTED, {
    name: 'QWEN_MEM_CITE_NUDGE_MIN_INJECTED',
    defaultValue: 5,
    min: 0,
  });
  return (
 ...
 * `runtime/cite-recall-dev--qwen-mem-lite.json` read lowStreak 58 for a project with
...
// Env opt-outs:
//   • QWEN_MEM_NO_CITE_NUDGE=1 — disables BOTH gates (full silence)
//   • QWEN_MEM_CITE_NUDGE_THRESHOLD — ratio gate threshold (default 0.4)
//   • QWEN_MEM_CITE_NUDGE_WIDE_DENOMINATOR=1 — judge the wide computeCiteRecall
//     ratio instead of the hook-injected one (pre-v6.6.0 behaviour)
//   • QWEN_MEM_CITE_NUDGE_MIN_INJECTED — ratio gate min-volume (default 5)
//   • QWEN_MEM_CITE_NUDGE_SILENCE_AFTER — consecutive-low streak before the
//     ratio nag self-silences (default 3; 0 = never silence)
export function buildCiteRecallNudge(project, runtimeDir, env = process.env) {
  if (env.QWEN_MEM_NO_CITE_NUDGE === '1') return '';
  try {
    ...
    const silenceAfter = envNumber(env.QWEN_MEM_CITE_NUDGE_SILENCE_AFTER, {
      name: 'QWEN_MEM_CITE_NUDGE_SILENCE_AFTER',
