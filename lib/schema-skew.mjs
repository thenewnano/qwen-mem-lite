...
export const SCHEMA_SKEW_CODE = 'QWEN_MEM_SCHEMA_TOO_NEW';

// PER PROJECT. The first version of this dedup used one marker file for the whole data dir,
// keyed on a per-project session id — so two projects sharing ~/.qwen-mem-lite overwrote
 ...
 * (EROFS, ENOSPC, a dismounted QWEN_MEM_DIR) made the catch block itself throw. Two-arm
...
const SKEW_MESSAGE_RE = /DB schema is v(\d+) but this qwen-mem-lite binary supports up to v(\d+)/;
const SKEW_MESSAGE_LOOSE_RE = /DB schema is v\d+/;

/**
 * True when `err` means "this DB was written by a newer qwen-mem-lite".
 *
...
export function schemaSkewRemedy({
  ...
  // then printed `qwen-mem-lite self-update` under a line reading "the code running here
  ...
  if (managed) {
    return {
      kind: 'managed',
      commands: ['qwen-mem-lite self-update'],
  ...
  return {
    kind: 'unknown',
    commands: [],
    note: 'Could not identify this install: no managed code install in ~/.qwen-mem-lite and no active plugin cache version. Run `qwen-mem-lite doctor` from the install you actually use.',
  };
}

/**
 * The user-facing block. Kept short on purpose — at SessionStart it shares one stdout
 * envelope with the startup dashboard and the `<qwen-mem-context>` block.
 *
...
export function formatSchemaSkewNotice({ dbVersion, binaryVersion, remedy, codeHome }) {
  ...
  const lines = [
    '⚠️ [qwen-mem-lite] Memory is OFF: this database was written by a newer version.',
...
const PROBE_BEGIN = '<<qwen-mem-schema-probe>>';
const PROBE_END = '<</qwen-mem-schema-probe>>';
